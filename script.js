...
function fetchForecast(ciudad) {
    const url = `https://api.openweathermap.org/data/2.5/forecast?q=${ciudad}&appid=${datosAPI.key}&lang=es`;
    fetch(url)
        .then(res => res.json())
...
function mostrarPronostico(data) {
    ...
    dias.forEach(dia => {
        ...
        contenedor.innerHTML += `
            <div class="dia">
                <p>${dia}</p>
                <img src="https://openweathermap.org/img/wn/${iconoDia}@2x.png" alt="${descripcion}">
                <p>${tempMedia}°C</p>
